...
import { getHistoryDrawTracking, postCreateProject, postDrawapproval, putDrawRequest } from '../../../services/api';
...
export const DrawRequestModal = ({loanId,isModalVisibleDetails,setIsModalVisibleDetails,drawRequestData,setDrawRequestData}) => {
                    ...
                    <Draggable key={column.key} draggableId={column.key} index={index}>
                      {(provided) => (
                        <th 
                          ...
                          className={styles.th}
                        >
                          {/* Conditionally format numeric headers */}
                          {['funded_amount', 'other_numeric_key'].includes(column.key) ? (
                            formatNumberWithCommas(column.title)
                          ) : (
                            formatNumberWithCommas(column.title)
                          )}

                          {/* Conditional rendering for edit icon */}
                          {role === 'lender' && column.key === 'funded_amount' && (
                            <img
                              src={editIcon}
                              alt="Edit Icon"
                        ...
                        <td className={styles.td} key={column.key}>
                          {editIndex === index && editField === column.dataIndex ? (
                            ...
                            <span onClick={() => handleEditClick(index, column.dataIndex, data[column.dataIndex])}>
                              {/* Apply conditional comma formatting based on dataIndex */}
                              {column.dataIndex === "requested_date"
                                ? formatDateTime(data[column.dataIndex])
                                : ["total_released_amount", "total_budget_amount", "total_funded_amount", "total_balance_amount", "total_draw_amount"].includes(column.dataIndex)
