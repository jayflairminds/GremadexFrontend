...
export const DocUpload = ({ loanId }) => {
  ...
  const handleDownload = async (file_id, document_name) => {
    try {
  ...
  const handleView = async (file_id, document_name) => {
    try {
      console.log("Downloading file with ID:", file_id);
      const res = await getViewFile(file_id); // Fetch file data from API
                  ...
                  <tbody>
                    {documentList[heading]?.map((doc, rowIndex) => (
                      ...
                      >
                        ...
                        <td className={stylesD.td}>
                          <button onClick={() => handleDownload(doc.file_id, doc.document_name)} className={stylesD.filledBtn}>
                            Download
                          </button>
                        </td>
                        <td className={stylesD.td}>
                          <button onClick={() => handleView(doc.file_id, doc.document_name)} className={stylesD.filledBtn}>
                            View
                          </button>
                        </td>
                        ...
                        {role === "borrower" && (
                         ...
                         <td className={stylesD.td}> 
                         <button  onClick={() => handleSummaryClick(doc.file_id)} className={stylesD.filledBtn}>
                          Summary
                        </button>
